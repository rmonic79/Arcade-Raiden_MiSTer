...
case (r_cond)
    ...
    C_REP:    begin
                  ...
                  // ── PATCH LOCALE 2026-08-18 (DEVIAZIONE DA UPSTREAM, da
                  // segnalare a wickerwaka) ────────────────────────────────
                  // L'ESITO DELL'ITERAZIONE APPENA COMPLETATA VA ONORATO
                  // PRIMA DEL RITIRO PER INTERRUPT.  Nell'ordine originale il
                  // ramo di ritiro precedeva la valutazione di `rep_test`: un
                  // INT pendente sull'iterazione che TROVA il match (REPNE
                  // SCASB sul terminatore, REPE sul mismatch) ritirava la REP
                  // con DI/CX gia' avanzati e il match MAI consultato -> al
                  // rientro la scansione riprendeva OLTRE il terminatore.
                  // Misurato: strlen del punteggio Raiden torna 12 invece di
                  // 6 -> stampa dei byte di stack nelle 2 celle dopo i campi
                  // (repro: scasb_bench, 10 fasi INT su 600; identico su HEAD
                  // e su e61f2e09).  Il silicio onora il test: la REP termina
                  // normalmente e l'INT si serve al confine di istruzione
                  // successivo.  Il corpus upstream (INT.F3AA = REP STOSB,
                  // TEST_NONE) non esercita mai questo caso; per TEST_NONE
                  // questo riordino e' BIT-IDENTICO all'originale.
                  // Doc: docs/BUG_REP_SCASB_INT_2026-08-18.md
                  else if ((rep_test_n == TEST_Z) &&
                           (psw_n[FZ] != rep_pol_n))
                      taken = 1'b0;                // match: retire normale
